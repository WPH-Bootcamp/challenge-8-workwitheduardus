            ...
            <div className="flex flex-row lg:flex-col gap-2 lg:gap-6 lg:w-[200px] shrink-0">
                {industries.map((ind) => {
                    ...
                    return (
                        ...
                        } className={["text-left text-base font-semibold transition-colors cursor-pointer", "flex items-center gap-3", isActive ? "text-black": "text-neutral-500 hover:text-neutral-300",].join(" ")}>
                        <span className={["w-1 h-6 rounded-full shrink-0 transition-colors", isActive ? "bg-primary-200" : "bg-neutral-700",].join(" ")}/>
                            {ind.label}
                        </button>
                    );
                })}
...
export default InddustrySection;